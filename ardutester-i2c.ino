...
const unsigned char VERSION_str[] MEM2_TEXT = "Ttester 1.08.001";
...
void lcd_set_cursor(uint8_t row, uint8_t col)
{
  ...
  lcd_command(CMD_SetDDRAMAddress | (col + row_offsets[row]));
  uart_newline();
}
...
void lcd_pgm_custom_char(uint8_t location, const unsigned char *chardata) { 
  location &= 0x7;
  lcd_command(CMD_SetCGRAMAddress | (location << 3));
